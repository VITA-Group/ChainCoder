...
class TransformerModel(nn.Module):
    ...
    def __init__(self, params, vocabulary_defs, is_encoder, with_output, use_prior_embeddings, positional_embeddings, is_sample_embedder=False, pad_index=-1):
        """
        ...
        if is_encoder:
            self.n_words = len(vocabulary_defs.i2t_inp)
        else:
            self.n_words = len(vocabulary_defs.i2t_out)
            self.pad_index = vocabulary_defs.output_pad_id
            self.eos_index = vocabulary_defs.eos_token_id
            self.bos_index = vocabulary_defs.bos_token_id



        # model parameters
        self.dim = (
            params.arch_encoder_dim if is_encoder else params.arch_decoder_dim
        )  # 512 by default
        self.src_dim = params.arch_encoder_dim
        ...
        # self.n_layers = params.arch_encoder_layers if is_encoder else params.arch_decoder_layers
        if is_encoder:
            if is_sample_embedder:
                self.n_layers = params.arch_sampEmbedder_layers
            else:
                self.n_layers = params.arch_encoder_layers
        else:
            self.n_layers = params.arch_decoder_layers
        ...
        if positional_embeddings is None or positional_embeddings == "alibi":
           self.position_embeddings = None
            ...
            self.position_embeddings = Embedding(params.transformer_max_seq_len, self.dim)
            create_sinusoidal_embeddings(
                params.transformer_max_seq_len, self.dim, out=self.position_embeddings.weight
            )
    ...
    # def embed(self, tensor_x2: torch.Tensor) -> torch.Tensor:
    #     # 'x' in tensor_x2 means any number of dimensions here. For io data, it is ist2; for code data, it is it2.
    #     tensor_x2e = self.embeddings(tensor_x2)
    #     tensor_xe = tensor_x2e.sum(dim=-2)
    #     return tensor_xe
    ...
    def fwd(
        ...
        # slen, bs = x.size()[:2]  # TBE
        # x = x.transpose(0, 1)  # batch size as dimension 0

        bs, slen = x.size()[:2] # x shape is BTE
        ...
        if src_enc is not None: # shape should be: BTE
        ...
        if self.is_decoder and src_enc is not None:
            _max_tok_len = src_len.max()
            src_mask = (
                torch.arange(_max_tok_len, dtype=torch.long, device=lengths.device)
                < src_len[:, None]
            )
            src_enc = src_enc[:, :_max_tok_len]
    ...
    def predict(self, tensor, len2, code_BT):
        """
        ...
        _max_code_len = len2.max()
        alen = torch.arange(_max_code_len, dtype=torch.long, device=len2.device)
        pred_mask = (
            alen[:, None] < len2[None] - 1
        )  # shape is [max_tok_len, B] ; do not predict anything given the last target word
        code_BT = code_BT[:, :_max_code_len]
        tensor = tensor[:, :_max_code_len]

        y = code_BT.transpose(0,1)[1:].masked_select(pred_mask[:-1])
        assert len(y) == (len2 - 1).sum().item(), 'total num of labels should equal'
        # code_TB, len2, y = to_cuda(code_TB, len2, y)
        assert (y == self.pad_index).sum().item() == 0

        tensor = tensor.transpose(0,1)  # BT.. -> TB..
    ...
    def generate(self, src_enc, src_len, max_len=200, top_p=1.0, sample_temperature=None):
        ...
        # input batch
        bs = len(src_len)
        assert src_enc.size(0) == bs # should be BTE
        ...
        while cur_len < max_len:
            ...
            # compute word scores
            tensor_BTE = self.forward(
                "fwd",
                x=generated[:cur_len].transpose(0,1), # input must be BTE
                lengths=gen_len,
                positions=positions[:cur_len],
                causal=True,
                src_enc=src_enc,   # should be BTE
                src_len=src_len,
                use_cache=True,
            )
            tensor = tensor_BTE.transpose(0,1) # TBE
